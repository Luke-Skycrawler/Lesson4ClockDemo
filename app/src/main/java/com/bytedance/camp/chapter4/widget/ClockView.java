...
//        needlePaint.setStrokeWidth(UNIT_LINE_WIDTH);
    ...
    private void drawTimeNeedles(Canvas canvas) {
         ...
         */
        double rad=Math.toRadians(second*6-90);

        // int hourDegree = 180;

        float endY=(float)(centerY+radius*SECOND_NEEDLE_LENGTH_RATIO* Math.sin(rad));
        float endX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(rad));
        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,endX,endY,needlePaint);
        rad=Math.toRadians(6*minute-90);
        endY=(float)(centerY+radius*MINUTE_NEEDLE_LENGTH_RATIO* Math.sin(rad));
        endX = (float) (centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(rad));
        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        canvas.drawLine(centerX,centerY,endX,endY,needlePaint);
        rad=Math.toRadians(-90+30*hour+0.5*minute);
        endY=(float)(centerY+radius*HOUR_NEEDLE_LENGTH_RATIO* Math.sin(rad));
        endX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(rad));
